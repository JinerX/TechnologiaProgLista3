...
public class ReportBuilder {
	 ...
	 * volatile instance - the variable is always read from MM, not cached by threads
	 * prevents instruction reordering
	 * first if - impproves performance preventing threads from entering the synchronized block
	 * synchronized - ensures only a single thread can access the inside at a time
	 * second if - in case two threads asked for access at the same time the second thread will have update the instance as not null
	 * preventing it from calling it again
	 */
